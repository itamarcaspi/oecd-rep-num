...
p_R <- df_R_q %>% 
  ...
  annotate("label", x = last_date + days(5), y = last_oecd-0.1, label = label_oecd, color = "#6DCD59", size = 3.5) +
  annotate("label", x = last_date + days(5), y = last_israel, label = label_israel, color = "#3E4A89", size = 3.5) +
  labs(
    y = expression("R"[t]),
    x = "",
    ...
    color = "",
    lty = "",
    caption = glue("Notes: Gray shaded area denotes the 10-days between infection and outpatiant testing.\nData: {data_source_name}.")
  ) +
...
p_cases <- df_oecd_cases_q %>% 
  ...
  annotate("label", x = last_date + days(5), y = last_oecd, label = label_oecd, color = "#6DCD59", size = 3.5) +
  annotate("label", x = last_date + days(5), y = last_israel, label = label_israel, color = "#3E4A89", size = 3.5) +
  geom_vline(data = events, aes(xintercept = date), linetype = 2) +
...
p_tests <- df_oecd_tests_q %>%
  ...
  geom_vline(data = events, aes(xintercept = date), linetype = 2) +
  geom_text(data = events, aes(x=date, y=0, label=event), size=3, angle=90, vjust=-0.4, hjust=0) +
  annotate("label", x = last_date + days(2), y = last_oecd, label = label_oecd, color = "#6DCD59", size = 3.5) +
  annotate("label", x = last_date + days(2), y = last_israel, label = label_israel, color = "#3E4A89", size = 3.5) +
  labs(
