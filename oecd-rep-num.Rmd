...
p_tests <- df_oecd_tests_q %>%
  ...
  # annotate("text", x = last_date + days(2), y = last_oecd, label = label_oecd, color = "#6DCD59", size = 3.5) +
  # annotate("text", x = last_date + days(2), y = last_israel, label = label_israel, color = "#3E4A89", size = 3.5) +
  geom_vline(data = events, aes(xintercept = date), linetype = 2) +
  geom_text(data = events, aes(x=date, y=0, label=event), size=3, angle=90, vjust=-0.4, hjust=0) +
  labs(
